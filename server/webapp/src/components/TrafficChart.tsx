...
export default function TrafficChart({ users: propUsers }: TrafficChartProps) {
  ...
  const drawChart = () => {
    ...
    // Цвета из дизайн-токенов (читаем live из CSS-vars, чтобы реагировать на theme bridge)
    const uploadColor = readVar('--accent', '#38BDF8');
    const downloadColor = readVar('--violet', '#A78BFA');
    const textColor = readVar('--text-primary', '#E8EDF5');
    const hintColor = readVar('--text-tertiary', '#8090AB');
    ...
    users.forEach((user, idx) => {
      ...
      upGrad.addColorStop(1, 'rgba(56, 189, 248, 0.3)');
      ...
      dnGrad.addColorStop(1, 'rgba(167, 139, 250, 0.3)');
  ...
  const totalUp = users.reduce((s, u) => s + u.traffic_up, 0)
  const totalDown = users.reduce((s, u) => s + u.traffic_down, 0)
  const summary = `Трафик ${users.length} пользователей. Исходящий: ${formatBytes(totalUp)}. Входящий: ${formatBytes(totalDown)}.`
  ...
  return (
    <div className="traffic-chart">
      ...
      />
      <p className="sr-only">
        Гистограмма по {users.length} пользователям.{' '}
        {users.map(u => `${u.name}: исходящий ${formatBytes(u.traffic_up)}, входящий ${formatBytes(u.traffic_down)}`).join('. ')}
      </p>
