...
async def get_subscription(request: Request, token: str):
    ...
    headers = {
        ...
        "Content-Disposition": _build_content_disposition(f"{user.name}.txt"),
...
async def _build_routing_header() -> str | None:
    ...
    try:
        ...
        direct_domain_list = [
            ...
            "domain:twitch.tv",         # Twitch — direct (видео не грузятся через VPN)
            "domain:ttvnw.net",         # Twitch CDN
            "domain:jtvnw.net",         # Twitch CDN alt
            "domain:ext-twitch.tv",     # Twitch extensions
        ]
        if direct_domains:
