...
async def users_add_expiry(message: Message, state: FSMContext):
    ...
    info_text += (
        ...
        f"<code>{sub_url}</code>\n"
        f"<i>↳ внутри обе ссылки — VLESS и Hysteria2 (v2rayN / v2rayNG / Streisand)</i>\n\n"
...
async def user_link(callback: CallbackQuery):
    """Get direct VPN links (VLESS + Hysteria2) for user."""
    user_id = int(callback.data.split(":")[2])
    mgr = UserManager()
    user = await mgr.get_user(user_id)
    ...
    parts = [
        f"🔗 <b>Прямые ссылки для {user.name}:</b>\n",
        f"🛡 <b>VLESS + Reality</b>\n<code>{vless_url}</code>",
    ]

    if config.HYSTERIA_ENABLED and getattr(user, "hysteria_password", None):
        h_url = HysteriaConfigManager().generate_hysteria_url(
            user.email, user.hysteria_password, user.name
        )
        parts.append(f"⚡ <b>Hysteria2 / QUIC</b>\n<code>{h_url}</code>")

    parts.append(
        "<i>💡 Скопируйте и вставьте в клиент (v2rayN / v2rayNG / V2RayTun / Dem1chVPN).\n"
        "Если клиент поддерживает подписки — лучше используйте 📡 Подписка.</i>"
    )
