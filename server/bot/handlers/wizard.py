...
async def self_link(callback: CallbackQuery):
    ...
    await callback.message.answer(text, reply_markup=back_button("menu:main"))
...
async def wizard_step(callback: CallbackQuery):
    ...
    if step in platform_apps:
        ...
        hysteria_url = None
        if config.HYSTERIA_ENABLED and getattr(user, "hysteria_password", None):
            hysteria_url = HysteriaConfigManager().generate_hysteria_url(
                user.email, user.hysteria_password, user.name
            )
        ...
        if method == "link":
            link_text = (
                ...
                f"🛡 <b>VLESS + Reality</b> (TCP/443):\n"
                f"<code>{vless_url}</code>"
            )
            if hysteria_url:
                link_text += (
                    f"\n\n⚡ <b>Hysteria2 / QUIC</b> (UDP/8444):\n"
                    f"<code>{hysteria_url}</code>\n\n"
                    f"Импортируйте обе — в клиенте сможете переключаться, если на одном начнутся проблемы."
                )
            link_text += (
