...
async def on_startup(bot: Bot):
    ...
    # Бэкфилл Hysteria2 userpass из БД — для старых юзеров после апгрейда.
    # Без этого они есть в xray, но Hysteria их не знает.
    if config.HYSTERIA_ENABLED:
        try:
            mgr = UserManager()
            h_mgr = HysteriaConfigManager()
            cfg = await h_mgr._aread_config()
            existing = set((cfg.get("auth", {}).get("userpass") or {}).keys())
            users = await mgr.get_active_users()
            missing = [u for u in users if u.email not in existing and u.hysteria_password]
            if missing:
                # Пишем все сразу одним батчем, потом 1 рестарт
                auth = cfg.setdefault("auth", {"type": "userpass"})
                bucket = auth.setdefault("userpass", {}) or {}
                for u in missing:
                    bucket[u.email] = u.hysteria_password
                auth["userpass"] = bucket
                await h_mgr._awrite_config(cfg)
                await h_mgr.reload_hysteria()
                logger.info(f"✅ Hysteria2: backfilled {len(missing)} existing users")
        except Exception as e:
            logger.warning(f"Hysteria2 backfill skipped: {e}")
            ...
            for user in all_users:
                ...
                if stats and (stats["uplink"] > 0 or stats["downlink"] > 0):
                    ...
                    if (
                        ...
                        await hysteria_mgr.remove_client(updated_user.email)
...
async def main():
    ...
    # На случай если у токена висит webhook от прошлой инсталлы — иначе getUpdates падает с Conflict
    try:
