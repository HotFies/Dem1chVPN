...
class User(Base):
    ...
    hysteria_password = Column(String(64), nullable=False,
                               default=lambda: secrets.token_urlsafe(32))
    ...
    def is_expired(self) -> bool:
        if self.expiry_date is None:
            return False
        # SQLite в DateTime без timezone=True теряет tz — читаем как naive, считаем UTC
...
async def init_db():
    ...
    async with engine.begin() as conn:
        ...
        await _migrate_hysteria_password(conn)


async def _migrate_hysteria_password(conn):
    # create_all не добавляет колонки в существующие таблицы — делаем вручную
    from sqlalchemy import text
    cols = await conn.execute(text("PRAGMA table_info(users)"))
    names = {row[1] for row in cols.fetchall()}
    if "hysteria_password" in names:
        return
    await conn.execute(text("ALTER TABLE users ADD COLUMN hysteria_password VARCHAR(64)"))
    rows = await conn.execute(text("SELECT id FROM users WHERE hysteria_password IS NULL"))
    for (uid,) in rows.fetchall():
        await conn.execute(
            text("UPDATE users SET hysteria_password = :pw WHERE id = :id"),
            {"pw": secrets.token_urlsafe(32), "id": uid},
        )
