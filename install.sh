...
SYSCTL
    ...
    ufw allow 8444/udp     # Hysteria2 (QUIC)
    ufw --force enable
...
# ──── Шаг 4b: Установка бинаря Hysteria2 ────

install_hysteria_binary() {
    log_step "Шаг 4b: Установка бинаря Hysteria2"

    # Официальный скрипт apernet/hysteria — ставит бинарь и одноимённый системный сервис.
    # Свой сервис dem1chvpn-hysteria создаётся позже (см. create_services).
    bash <(curl -fsSL https://get.hy2.sh/)

    # Дефолтный hysteria-server сервис нам не нужен — отключаем, чтобы не конфликтовал.
    systemctl disable --now hysteria-server.service 2>/dev/null || true
    systemctl disable --now hysteria-server@*.service 2>/dev/null || true

    if ! command -v hysteria &>/dev/null; then
        log_error "Hysteria не удалось установить"
        exit 1
    fi
    ...
    echo "HYSTERIA_PORT=${HYSTERIA_PORT}" >> /tmp/dem1chvpn_keys
    echo "HYSTERIA_OBFS_PASSWORD=${HYSTERIA_OBFS_PASSWORD}" >> /tmp/dem1chvpn_keys

    log_info "Бинарь Hysteria2 установлен"
}
...
# Hysteria2 (параллельный протокол на UDP)
HYSTERIA_ENABLED=true
HYSTERIA_DOMAIN=${SUB_DOMAIN}
...
# ──── Шаг 6.95: Настройка Hysteria2 ────

configure_hysteria() {
    log_step "Шаг 6.95: Настройка Hysteria2"
    ...
    if [ ! -f "${DEM1CHVPN_DIR}/server/hysteria/config_template.yaml" ]; then
        log_warn "config_template.yaml не найден — пропускаю настройку Hysteria"
        sed -i 's/^HYSTERIA_ENABLED=true/HYSTERIA_ENABLED=false/' "$ENV_FILE" || true
        return 0
    fi
    ...
    # Дать боту dem1chvpn доступ на чтение сертификатов Caddy
    usermod -a -G caddy dem1chvpn 2>/dev/null || true

    rm -f /tmp/dem1chvpn_keys

    log_info "Hysteria2 настроена. Конфиг: /etc/hysteria/config.yaml"
}
...
SERVICE
    ...
    # Сервис Hysteria2
    # User=root нужен чтобы читать cert Caddy (chmod 600 caddy:caddy в /var/lib/caddy/...)
    # Альтернатива — ACL/chmod на cert, но Caddy их перезатирает при ротации.
    cat > /etc/systemd/system/dem1chvpn-hysteria.service << SERVICE
[Unit]
Description=Dem1chVPN Hysteria2 Server
...
# Hysteria2 restart раз в неделю — подхватить ротированный Let's Encrypt cert
...
main() {
    ...
    configure_hysteria || log_warn "Hysteria2 не настроена — продолжаем"
    setup_warp || {
